...
class _RacineAppState extends State<RacineApp> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("EDEN MOBILITY - Douala", style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
    ...
    switch (etape) {
      ...
      case EtapePassager.walletCheck:
        return EcranWalletCheck(
          ...
          onCreditUrgence: () {
            ...
            widget.onSoldeChange(-prixTotal);
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Cr̩dit d'urgence utilis̩ (-1200 FCFA de frais) - Course lanc̩e !"), backgroundColor: Color(0xFF1A6EBF)));
      ...
      case EtapePassager.suivi:
        return EcranSuiviCourse(
          ...
          onSOS: () => showDialog(context: context, builder: (_) => AlertDialog(title: const Text("SOS - Urgence"), content: const Text("Signal envoy̩ au centre de s̩curit̩ EDEN MOBILITY Douala.\nUn op̩rateur te rappelle dans 60 secondes.\nPosition partag̩e: Akwa - Bonapriso."), actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text("OK"))])),
          onArriveeForcee: () {
            timerSuivi?.cancel();
            setState(() => etape = EtapePassager.fin);
          },
        ...
        Card(
          child: Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text("Dur̩e estim̩e", style: TextStyle(color: Colors.grey.shade600)), Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: const Color(0xFF1A6EBF), borderRadius: BorderRadius.circular(20)), child: const Text("18 min", style: TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w700)))]),
            ]),
          ),
        ),
        const SizedBox(height: 12),
          ...
          Card(
            ...
            child: Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                const SizedBox(height: 8),
                Text("Solde insuffisant (${formatMontant(solde)} < ${formatMontant(prixTotal)}), mais tu es ̩ligible au cr̩dit d'urgence EDEN MOBILITY (client fid̬le).", style: TextStyle(color: Colors.orange.shade900, fontSize: 12)),
                const SizedBox(height: 6),
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        ...
        Center(child: Text("Merci d'avoir voyag̩ avec EDEN MOBILITY", style: TextStyle(color: Colors.grey.shade600))),
        const SizedBox(height: 16),
        Card(
          child: Padding(
            padding: const EdgeInsets.all(16),
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(onPressed: () { Navigator.pop(context); onSoldeChange(5000); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Recharge simul̩e +5000 FCFA !"), backgroundColor: Color(0xFF1A6EBF))); }, child: const Text("+5 000")),
